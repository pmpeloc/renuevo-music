...
import { X, Search, Video, Info, Youtube } from 'lucide-react';
...
export default function AddSongModal({
  ...
  async function handleSave() {
    ...
    if (!song && showNewForm) {
      ...
      if (!song) { setSaving(false); return; }
      setSelectedSong(song);
    }
  ...
  const ytId = youtubeUrl ? extractYoutubeId(youtubeUrl) : (selectedSong?.youtube_url ? extractYoutubeId(selectedSong.youtube_url) : null);
        ...
        <div className="px-5 py-4 space-y-5">
          ...
          {!editingSong && (
            <div>
                ...
                <div className="mt-2 border border-gray-100 rounded-xl overflow-hidden">
                  {catalogResults.map((song) => (
                    ...
                    >
                      ...
                      {song.youtube_url && (
                        <Video size={14} className="text-red-400 ml-auto flex-shrink-0" />
                      )}
                    </button>
                  ))}
                </div>
              ...
              {!showNewForm && (
                <button
                  onClick={() => { setShowNewForm(true); setSearchQuery(''); setCatalogResults([]); }}
            ...
            <div className="space-y-3">
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-sm" style={{ color: 'var(--purple-800)' }}>
                    {selectedSong.title}
                  </p>
                  {selectedSong.artist && (
                    <p className="text-xs" style={{ color: 'var(--purple-600)' }}>{selectedSong.artist}</p>
                  )}
                </div>
                {!editingSong && (
                  <button onClick={() => { setSelectedSong(null); setYoutubeUrl(''); }} className="flex-shrink-0">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-2.5 rounded-xl text-white text-xs font-medium flex-shrink-0"
          ...
          {showNewForm && !selectedSong && (
            <div className="space-y-3 p-4 bg-gray-50 rounded-2xl">
              <p className="text-sm font-medium text-gray-700">Nueva canción</p>
              <div>
                <label className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-1.5 block">Título *</label>
                <input
                  type="text"
                  value={newTitle}
              ...
              <div>
                <label className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-1.5 block">Artista</label>
                <input
                  type="text"
                  value={newArtist}
              ...
              <div>
                <label className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-1.5 block">Link YouTube</label>
                <div className="flex gap-2">
                  <input
                    type="url"
                    value={newYoutube}
                ...
                {newYoutube && extractYoutubeId(newYoutube) && (
                  <div className="mt-2 flex items-center gap-2">
                    <Image
                      ...
                      style={{ width: 80, height: 56 }}
                    />
                    <p className="text-xs text-gray-500">Vista previa detectada ✓</p>
                  </div>
                )}
              </div>
              <button
              ...
              {keyHistory && (
                <div className="flex items-start gap-2 p-3 bg-blue-50 rounded-xl">
                  ...
                  <p className="text-xs text-blue-700">
                    ...
                    {keyHistory.starts_in ? ` (comienza en ${keyHistory.starts_in})` : ''}{' '}
                    para esta canción.
                  </p>
                </div>
              )}
          ...
          >
            {saving ? 'Guardando...' : editingSong ? 'Guardar cambios' : 'Agregar a la lista'}
          </button>
        </div>
      </div>
    </div>
