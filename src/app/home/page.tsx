...
import {
  Service, ServiceType, ServiceMember, ServiceSong,
  SERVICE_LABELS,
} from '@/types';
import {
  getDateRange, hasServices, isSameDay, formatMonth,
...
export default function HomePage() {
  ...
  // Scroll automático a "hoy" al montar
  useEffect(() => {
  ...
  function handleStripScroll() {
    ...
    const itemWidth = 52;
    const centerIndex = Math.round((el.scrollLeft + el.clientWidth / 2) / itemWidth);
  ...
  const STATUS_CONFIG = {
    ...
    no_songs:    { label: 'Sin canciones',  bg: 'bg-amber-100',  text: 'text-amber-800' },
    no_director: { label: 'Sin director',   bg: 'bg-red-100',    text: 'text-red-800'   },
    empty:       { label: '',               bg: '',              text: ''               },
  };
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="px-4 py-1">
        <p className="text-sm font-medium capitalize" style={{ color: 'var(--purple-600)' }}>
        ...
        {dates.map((date, i) => {
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center py-2 px-2 rounded-xl transition-colors flex-shrink-0 w-12 ${
                isSelected
                  ? 'text-white'
                  : isToday
                  ? 'text-gray-900'
              ...
              style={isSelected ? { background: 'var(--purple-600)' } : undefined}
            >
              <span className="text-[10px] font-medium capitalize">{dayName.replace('.', '')}</span>
              <span className={`text-base font-semibold mt-0.5 ${isToday && !isSelected ? 'text-purple-600' : ''}`}>
                {dayNum}
              </span>
              {hasEv && (
                <div
                  className="w-1 h-1 rounded-full mt-1"
                  style={{ background: isSelected ? 'rgba(255,255,255,0.6)' : 'var(--purple-600)' }}
                />
              )}
            </button>
          );
                      ...
                      {coro.length > 0 && (
                        <>
                          ...
                          <div className="flex -space-x-1">
                            {coro.map((m) =>
                              m.profile ? <Avatar key={m.id} profile={m.profile} size="sm" /> : null
                            )}
                          </div>
